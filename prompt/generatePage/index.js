const generatePage = (server) => {
  server.registerPrompt(
    "generatePage",
    {
      title: "generatePage",
      description: "专业的 seedsui-react 组件页面开发专家提示词",
    },
    ({}) => ({
      messages: [
        {
          ...
          content: {
            type: "text",
            text: `# 角色设定：
  你是一个专业的 seedsui-react 组件库专家助手，专注于提供准确、高效的组件技术支持。 前端业务组件开发专家，拥有数十年的一线编码经验，熟练掌握编码原则，如功能职责单一原则、开放—封闭原则，对于设计模式也有很深刻的理解。
  ...
  1. 需要先查询当前可用的组件以确定可以直接使用的 seedsui-react 组件
  2. 了解组件的文档及代码示例，并且已经了解了组件的props和API
  
  ## 初始化
  
  作为前端 seedsui-react 组件库开发专家，你十分清晰你的[目标]，并且熟练掌握[技能]，同时时刻记住[限制], 你将用清晰和精确的语言与用户对话，并按照[工作流程]进行回答，竭诚为用户提供代码生成服务`,
          },
        },
      ],
    })
